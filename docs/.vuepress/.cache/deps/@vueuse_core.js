...
function useDateFormat(date, formatStr = "HH:mm:ss") {
  return computed(() =>
    formatDate(normalizeDate(unref(date)), unref(formatStr))
  );
}
      ...
      useEventListener(window2, "blur", (event) => {
        ...
        if (
          ((_a2 = document.activeElement) == null ? void 0 : _a2.tagName) ===
            "IFRAME" &&
          !(el == null ? void 0 : el.contains(document.activeElement))
        )
          handler(event);
      }),
  ].filter(Boolean);
...
function onKeyDown(key, handler, options = {}) {
  return onKeyStroke(
    key,
    handler,
    __spreadProps$8(__spreadValues$j({}, options), { eventName: "keydown" })
  );
}
function onKeyPressed(key, handler, options = {}) {
  return onKeyStroke(
    key,
    handler,
    __spreadProps$8(__spreadValues$j({}, options), { eventName: "keypress" })
  );
}
function onKeyUp(key, handler, options = {}) {
  return onKeyStroke(
    key,
    handler,
    __spreadProps$8(__spreadValues$j({}, options), { eventName: "keyup" })
  );
}
...
function useBase64(target, options) {
  ...
  if (isRef(target)) watch(target, execute, { immediate: true });
  else execute();
  return {
    base64,
    promise,
...
function useMediaQuery(query, options = {}) {
  ...
  const update = () => {
    if (!isSupported) return;
    if (!mediaQuery) mediaQuery = window2.matchMedia(query);
...
function useDark(options = {}) {
  ...
  const mode = useColorMode(
    __spreadProps$7(__spreadValues$g({}, options), {
...
function useRefHistory(source, options = {}) {
  ...
  const manualHistory = useManualRefHistory(
    source,
    __spreadProps$62(__spreadValues$f({}, options), {
...
function useDebouncedRefHistory(source, options = {}) {
  ...
  const history = useRefHistory(
    source,
    __spreadProps$52(__spreadValues$e({}, options), { eventFilter: filter })
  );
...
function useDevicePixelRatio({ window: window2 = defaultWindow } = {}) {
  ...
  useEventListener(window2, "resize", handleDevicePixelRatio, {
    passive: true,
  });
...
function useDisplayMedia(options = {}) {
  ...
  const isSupported = Boolean(
    (_b = navigator == null ? void 0 : navigator.mediaDevices) == null
      ? void 0
      : _b.getDisplayMedia
  );
...
function useElementVisibility(
  ...
  if (window2)
    tryOnMounted(() =>
      useEventListener(unref(scrollTarget) || window2, "scroll", testBounding, {
        capture: false,
        passive: true,
      })
    );
    ...
    if (args.length > 0) {
      if (isFetchOptions(args[0])) {
        ...
        fetchOptions = __spreadProps$32(
          __spreadValues$b(__spreadValues$b({}, fetchOptions), args[0]),
          {
            headers: __spreadValues$b(
              __spreadValues$b({}, headersToObject(fetchOptions.headers) || {}),
...
function useFetch(url, ...args) {
  ...
  const execute = async (throwOnFailed = false) => {
    ...
    const context = {
      url: unref(url),
      options: __spreadValues$b(
        __spreadValues$b({}, defaultFetchOptions),
        fetchOptions
      ),
      cancel: () => {
        isCanceled = true;
...
function useFileDialog(options = {}) {
  ...
  const open = (localOptions) => {
    if (!input) return;
    const _options = __spreadValues$a(
      __spreadValues$a(__spreadValues$a({}, DEFAULT_OPTIONS), options),
      localOptions
    );
...
function useFileSystemAccess(options = {}) {
  ...
  async function open(_options = {}) {
    ...
    const [handle] = await window2.showOpenFilePicker(
      __spreadValues$92(__spreadValues$92({}, unref(options)), _options)
    );
  ...
  if (element) {
    ...
    const onScrollHandler = (e) => {
      ...
      arrivedState.right =
        ...
        eventTarget.scrollWidth - (offset.right || 0);
      x.value = scrollLeft;
      ...
      arrivedState.bottom =
        ...
        eventTarget.scrollHeight - (offset.bottom || 0);
      y.value = scrollTop;
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  const state = reactive(
    useScroll(
      element,
      __spreadProps$22(__spreadValues$72({}, options), {
...
var __spreadValues$62 = (a, b) => {
  for (var prop in b || (b = {}))
    if (__hasOwnProp$72.call(b, prop)) __defNormalProp$62(a, prop, b[prop]);
  if (__getOwnPropSymbols$72)
    for (var prop of __getOwnPropSymbols$72(b)) {
      if (__propIsEnum$72.call(b, prop)) __defNormalProp$62(a, prop, b[prop]);
    }
  return a;
};
...
function useShare(shareOptions = {}, options = {}) {
  ...
  const share = async (overrideOptions = {}) => {
    if (isSupported) {
      const data = __spreadValues$32(
        __spreadValues$32({}, unref(shareOptions)),
...
function useThrottledRefHistory(source, options = {}) {
  ...
  const history = useRefHistory(
    source,
    __spreadProps2(__spreadValues$22({}, options), { eventFilter: filter })
  );
...
function useTimeAgo(time, options = {}) {
  ...
  function getTimeago(from, now22) {
    ...
    if (typeof max === "string") {
      const unitMax =
        (_a22 = UNITS.find((i) => i.name === max)) == null ? void 0 : _a22.max;
...
function useUserMedia(options = {}) {
  ...
  const { navigator = defaultNavigator } = options;
  const isSupported = Boolean(
    (_c = navigator == null ? void 0 : navigator.mediaDevices) == null
      ? void 0
      : _c.getUserMedia
  );
  const stream = shallowRef();
...
function useVModel(props, key, emit, options = {}) {
  var _a2, _b, _c, _d, _e;
  const { passive = false, eventName, deep = false, defaultValue } = options;
  const vm = getCurrentInstance();
  const _emit =
    emit ||
